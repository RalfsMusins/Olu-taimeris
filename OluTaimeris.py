...
def pong_loop():
    ...
    if ball_y <= 0 or ball_y >= PONG_HEIGHT:
        ball_dy *= -1
...
def draw_pong():
    ...
    canvas.create_rectangle(5, left_paddle_y, 5+PADDLE_W, left_paddle_y+PADDLE_H, fill="#00AEFF")
...
timers = {
    "soft": {"name": "Mīksti","time": SOFT_BOILED_SECONDS,"color": COLOR_SOFT},
    "medium": {"name": "Vidēji","time": MEDIUM_BOILED_SECONDS,"color": COLOR_MEDIUM},
    "hard": {"name": "Cieti","time": HARD_BOILED_SECONDS,"color": COLOR_HARD},
}
...
label = tk.Label(window, text="00:00",
...
tk.Label(window,
         text="Zilais taisnstūris, W un S lai kustētos",
         font=("Helvetica", 9),
         bg=COLOR_BG, fg=COLOR_TEXT).pack(pady=5)
